...
with st.form("declaration_form"):
    ...
    with col2:
        ...
        spec_desc = st.text_area("特殊規格與功能說明", "內建 Bluetooth 模組、可充電式鋰電池")
...
if submit_btn or "has_run" in st.session_state:
    ...
    for key, data in TAX_DATABASE.items():
        if key in prod_name:
            matched_data = data
            break
    ...
    st.divider()
    st.subheader(f"📊 AI 分析與比對結果：【{prod_name}】")
    ...
    # 價格偏離度比對 (需求 3)
    if diff_rate < -20:
        st.error(f"⚠️ **價格風險警示**：您申報的單價 (TWD {prod_price}) **低於市場完稅均價 {abs(diff_rate):.1f}%** (推估完稅均價為 TWD {est_cif:.0f})，易引發海關 C3 查驗並要求補件證明。")
    elif diff_rate > 20:
        st.info(f"💡 **價格提醒**：您申報的單價 (TWD {prod_price}) 高於市場完稅均價 {diff_rate:.1f}%。")
    else:
